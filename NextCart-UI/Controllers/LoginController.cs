...
namespace NextCart_User_UI.Controllers
{
    public class LoginController : Controller
    {
        ...
        // MODIFIED ACTION: RegisteredUsers to include search
        [Authorize(Roles = "ADMIN")]
        [HttpGet]
        public async Task<IActionResult> RegisteredUsers(string searchTerm)
        {
            // Store the current search term to pre-fill the search box
            ViewData["CurrentFilter"] = searchTerm;

            var customers = await _userService.GetCustomersAsync(); // Get all customers first
            ...
            if (!string.IsNullOrEmpty(searchTerm))
            {
                ...
                customers = customers
                            .Where(c => (c.UserName != null && c.UserName.ToLower().Contains(lowerSearchTerm)) ||
        ...
        /// NEW ACTION: ViewUserDetails(for viewing only)
[Authorize(Roles = "ADMIN")]
        [HttpGet]
        public async Task<IActionResult> ViewUserDetails(string id)
        {
            ...
            // Fetch the user using UserManager for comprehensive user details
        ...
        public async Task<IActionResult> UpdateProfile(User model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                if (updatedUser != null)
                {
                    // Refresh the user's authentication cookie with the updated claims (including UserName)
